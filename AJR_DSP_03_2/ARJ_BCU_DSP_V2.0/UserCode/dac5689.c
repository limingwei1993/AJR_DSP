...
{
    int j;
    short TempInput = 0x00;
    gioSetBit(spiPORT1,SPI_PIN_CS1,0);//SYNC_L;
    delay_us(50);
        for(j=0; j<16; j++)
        {
            gioSetBit(spiPORT1,SPI_PIN_CLK,0);//SPI1_CLK_L;
            ...
            gioSetBit(spiPORT1,SPI_PIN_CLK,1);//SPI1_CLK_H;
            TempInput<<=1;
            if(gioGetBit(spiPORT1,SPI_PIN_SOMI))//SPI1_MISO
            TempInput++;
            delay_us(100);
            /************READ DATA ENDS***********/
        }
    delay_us(50);
    gioSetBit(spiPORT1,SPI_PIN_CLK,0);//SPI1_CLK_L;
    gioSetBit(spiPORT1,SPI_PIN_CS1,1);/*SYNC_H*/;
    if((TempInput&0x04)==0)
        return TempInput;
    else
        return -1;
 ...
 * *左轮温度AD采集值
 ******************************/
short SpiRead_Left(void)
{
    int j;
    short TempInput = 0x00;
    gioSetBit(spiPORT1,SPI_PIN_CS0,0);//SYNC_L;
    delay_us(50);
        for(j=0; j<16; j++)
        {
            gioSetBit(spiPORT1,SPI_PIN_CLK,0);//SPI1_CLK_L;
    ...
    delay_us(50);
    gioSetBit(spiPORT1,SPI_PIN_CLK,0);//SPI1_CLK_L;
    gioSetBit(spiPORT1,SPI_PIN_CS0,1);/*SYNC_H*/;
    if((TempInput&0x04)==0)
        return TempInput;
    else
        return -1;
